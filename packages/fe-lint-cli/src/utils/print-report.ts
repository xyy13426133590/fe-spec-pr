// 1. 遍历每个文件的 messages
// 2. 用 text-table 对齐：行:列 | error/warning | message | rule
// 3. 用 chalk 着色，terminal-link 给规则文档加链接
// 4. 底部汇总：✖ N problems (x errors, y warnings)
// 5. fix 模式额外提示：无法自动修复的问题需手动处理
...
type ScanMessage = ScanResult['messages'][number];
type SummaryColor = 'red' | 'yellow';
...
export default (results: ScanResult[], fix: boolean): void => {
  ...
  let summaryColor: SummaryColor = 'yellow';
  ...
  // y error and z warnings potentially fixable with the `fe-lint-cli fix`
  //
  // ✔ no problems
  if (!fix && total > 0) {
